# -*- coding: utf-8 -*-
"""
SR King Bot Hosting Platform — full rewrite with security & reliability fixes.
Python 3.10+ required.

Only BOT_TOKEN comes from environment. Everything else is code config below.
"""
...
# ─────────────────────────────────────────────────────────────────
# CONFIG  (token only from env; everything else hardcoded here)
# ─────────────────────────────────────────────────────────────────
TOKEN = os.environ.get("BOT_TOKEN", "").strip()
if not TOKEN:
    raise RuntimeError(
        "BOT_TOKEN environment variable set nahi hai. "
        "Render/hosting ke Environment section me daalo."
    )
...
# Private backup channel — bot must be admin here.
...
# ─────────────────────────────────────────────────────────────────
# [FIXED: Network Isolation / Outbound Connection Security]
# Sandbox directory + auto-generated sitecustomize.py.
# Child Python processes pick this up via PYTHONPATH and get their
# socket module patched: DNS/connect to non-whitelisted hosts is
# blocked (or logged, depending on SRK_SANDBOX_MODE). Telegram API
# domains are whitelisted by default so hosted Telegram bots still
# work. No effect on the parent bot.py process.
# ─────────────────────────────────────────────────────────────────
SANDBOX_DIR = os.path.join(BASE_DIR, "_sandbox")
SANDBOX_MODE = "block"               # "block" | "warn" | "off"
SANDBOX_ALLOW_HOSTS = (
...
# Patches socket.getaddrinfo / socket.connect to enforce an outbound
# network whitelist. Controlled by:
#   SRK_SANDBOX_MODE       = "block" | "warn" | "off"
#   SRK_SANDBOX_ALLOW_HOSTS = comma-separated whitelist of host suffixes
import os
import sys
...
_ALLOWED = [
    ...
    for h in os.environ.get(
        ...
        "api.telegram.org,core.telegram.org,telegram.org,t.me,localhost,127.0.0.1,::1",
    ).split(",")
    if h.strip()
]
...
# Thread-local connection pool. Each thread gets its own SQLite
# connection that is created once and reused — no more
# open/close churn on every operation. WAL journal mode is enabled
# for better concurrent read/write behaviour.
# ─────────────────────────────────────────────────────────────────
...
def load_data():
    ...
    try:
        ...
        logger.info("Loaded: %d users, %d subs, %d admins.",
                    len(active_users), len(user_subscriptions), len(admin_ids))
    except Exception as e:
...
# ─────────────────────────────────────────────────────────────────
# [FIXED: Network Isolation / Outbound Connection Security]
# Child env now exports PYTHONPATH pointing at the sandbox dir +
# SRK_SANDBOX_* policy variables, so every hosted script's Python
# interpreter auto-loads sitecustomize.py (written at startup) and
# gets its socket layer restricted to the whitelist.
# ─────────────────────────────────────────────────────────────────
...
def _child_env(extra: Optional[dict] = None) -> dict:
    ...
    # [FIXED: Network Isolation / Outbound Connection Security]
    # Point child interpreters at the sandbox dir so sitecustomize.py
    # is auto-imported; policy vars configure its behaviour.
...
def _run_subprocess_and_track(kind: str, script_path: str, owner: int,
    ...
    if process.poll() is not None:
        ...
        safe_reply(message,
            f"❌ {md_code(file_name)} exited right after starting (code {code}).\n"
...
# ─────────────────────────────────────────────────────────────────
# DATABASE OPS  — [FIXED: Database Connection Overhead]
# Each function now uses the thread-local connection from `_db_conn()`.
# `conn.close()` calls have been removed because the connection is
# pooled and reused. `conn.commit()` still runs after every write,
...
def create_control_buttons(owner_id: int, file_name: str, running: bool):
    ...
    if running:
        ...
        markup.row(cbtn("📤 Send Command", style="primary",
        ...
        markup.row(cbtn("📜 View Logs", style="primary",
...
def _logic_send_welcome(message):
    ...
    if user_id not in active_users:
        ...
        try:
            note = (
                f"🎉 New user!\n👤 Name: {esc(uname)}\n"
                f"✳️ User: @{esc(uhandle) if uhandle else 'N/A'}\n"
                f"🆔 ID: {md_code(user_id)}\n📝 Bio: {esc(user_bio)}"
            )
            safe_send(OWNER_ID, note)
            if photo_id:
                try:
                    bot.send_photo(OWNER_ID, photo_id,
                                   caption=f"Pic of new user {user_id}")
                except Exception:
                    pass
        except Exception as e:
    ...
    txt = (
        f"〽️ Welcome, {esc(uname)}!\n\n"
        f"🆔 Your ID: {md_code(user_id)}\n"
        f"✳️ Username: {md_code('@' + uhandle if uhandle else 'Not set')}\n"
...
def _logic_send_command(message):
    ...
    with _lock_state:
        locked = bot_locked
    if locked and not is_admin(message.from_user.id):
        safe_reply(message, "⚠️ Bot locked.")
        return
...
# ─────────────────────────────────────────────────────────────────
# FILE UPLOAD  — [FIXED: Blocking I/O on Main Thread]
# Validations (cheap, DB reads) run on the main thread so the user
# gets fast feedback for obvious rejections. Once validated, the
# network download + malware scan + ZIP extraction + npm/pip install
# + subprocess launch are offloaded to a background worker thread,
# so the main polling loop stays responsive for other users.
# ─────────────────────────────────────────────────────────────────
def _process_upload_worker(message):
    """Background worker: heavy I/O for a validated document upload."""
    user_id = message.from_user.id
    chat_id = message.chat.id
    ...
    try:
        try:
            ...
            safe_send(OWNER_ID, f"⬆️ '{esc(file_name)}' from {esc(message.from_user.first_name)} "
                                f"({md_code(user_id)})")
        except Exception as e:
...
def handle_file_upload_doc(message):
    ...
    # ── Cheap validations on the main thread (no I/O) ──
    if _block_banned(message):
        return
    with _lock_state:
        locked = bot_locked
    ...
    # [FIXED: Blocking I/O on Main Thread]
    # Hand off the heavy work (forward, download, scan, extract, npm/pip,
    # subprocess launch) to a background worker so the polling loop
    # stays free for other users' commands.
...
def cb_logs(call):
    ...
    try:
        ...
        if size == 0:
            ...
            with open(log_path, "r", encoding="utf-8", "ignore") as f:
                content = f.read()
        max_len = 3500
...
# ─────────────────────────────────────────────────────────────────
# CLEANUP  — [FIXED: Database Connection Overhead] closes pooled
# connections on shutdown; existing script-kill + final backup logic
# unchanged.
# ─────────────────────────────────────────────────────────────────
...
def cleanup():
    ...
    # [FIXED: Database Connection Overhead] Close all pooled connections
    try:
        _close_all_db_conns()
    except Exception as e:
