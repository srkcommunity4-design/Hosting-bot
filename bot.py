...
def self_ping_loop():
    """Render free web services sleep after ~15 min without inbound HTTP
    traffic. Ping our own public URL every 50 sec so that never happens."""
    import urllib.request
    time.sleep(30)  # let Flask come up first
    while True:
        ...
        if url:
            ...
            try:
                urllib.request.urlopen(
                    f"http://127.0.0.1:{os.environ.get('PORT', 10000)}/health",
                    timeout=10).read()
            except Exception:
                pass
        time.sleep(50)
...
def keep_alive():
    ...
    print("Flask Keep-Alive server + self-ping started.")
# --- End Flask Keep Alive ---

# --- Configuration ---
# Bot token now comes from an environment variable (Render: Environment ->
# BOT_TOKEN=<your token>). Nothing else about how TOKEN is
# used below changes.
TOKEN = os.environ.get("BOT_TOKEN")
if not TOKEN:
    raise RuntimeError(
        "BOT_TOKEN environment variable is not set. "
        "On Render: open your service -> Environment -> add BOT_TOKEN."
    )

OWNER_ID = 7892255798            # SR King - Telegram user ID
ADMIN_ID = 7892255798            # SR King - Telegram user ID
YOUR_USERNAME = '@srking5306'    # Personal contact / admin username
UPDATE_CHANNEL = '@srk_era'      # Updates channel username
FORCE_JOIN_CHANNELS = {
    "@srk_era": "🟢 JOIN",
}

# --- Backup Channel (auto database backup/restore) ---
# A PRIVATE Telegram channel where THIS BOT is an ADMIN. Every time the
# bot's data changes, it zips the database + everyone's uploaded scripts
# and pushes the zip here (pinned). On every restart/redeploy it looks
# for that pinned zip and restores from it automatically - so a Railway
# redeploy (which wipes the container's disk) never loses data. If the
# channel is empty, the bot just starts fresh, exactly like before.
BACKUP_CHANNEL_ID = -1003972335264   # SR King - private backup channel (bot must be admin here)
BACKUP_ZIP_NAME = "srk_bot_backup.zip"
BACKUP_DEBOUNCE_SECONDS = 8  # batch rapid-fire saves into one upload
...
# --- Colorful Buttons (real Bot API 9.4+ feature, not an emoji trick) ---
# Bot API 9.4 (Feb 2026) added a "style" field to KeyboardButton and
# InlineKeyboardButton: style="success" -> green, style="danger" -> red,
# style="primary" -> blue. Leaving style=None keeps Telegram's normal grey
# button. We write "style" into the outgoing JSON ourselves (instead of
# depending on it being an accepted constructor keyword) so this works
# whether or not the installed pyTelegramBotAPI build already has the
# parameter wired up.
...
def cbtn(text, style=None, **kwargs):
    """Inline keyboard button, optionally colored: 'success' green / 'danger' red / 'primary' blue."""
    return _apply_style(types.InlineKeyboardButton(text, **kwargs), style)

def kbtn(text, style=None):
    """Reply keyboard button, optionally colored: 'success' green / 'danger' red / 'primary' blue."""
    return _apply_style(types.KeyboardButton(text), style)

# --- Command Button Layouts (ReplyKeyboardMarkup) --- each entry is (label, style)
COMMAND_BUTTONS_LAYOUT_USER_SPEC = [
    [("📢 Updates Channel", "primary")],
    [("📤 Upload File", "success"), ("📂 Check Files", "primary")],
    [("⚡ Bot Speed", "primary"), ("📊 Statistics", "primary")],
    [("📤 Send Command", "primary"), ("📞 Contact Owner", "primary")]  # Added Send Command
]
ADMIN_COMMAND_BUTTONS_LAYOUT_USER_SPEC = [
    ...
    [("📤 Send Command", "primary"), ("👑 Admin Panel", "primary")],  # Added Send Command
...
# --- Ban / Lock helpers ---
# To ban someone just add their numeric ID to banned_users (or their
# username, with or without @, to banned_usernames) near the top of the file.
def _md_escape(text):
    """Escape characters that break Telegram legacy Markdown (names like 'Rahul_99')."""
    return re.sub(r'([_*`\[])', r'\\\1', str(text if text is not None else ''))

def is_banned_user(user_id, username=None):
    """True if this user is banned. The OWNER can never be banned."""
    if user_id == OWNER_ID:
        return False
    ...
    if username:
        ...
        for b in banned_usernames:
            if str(b).lstrip('@').lower() == uname:
                return True
    return False

def _block_banned(message, user=None):
    """Returns True (after telling the user) when the sender is banned."""
    try:
...
def _block_locked(message, user_id, text="⚠️ Bot locked by admin. Try later."):
    """Returns True (after telling the user) when the bot is locked and user is not an admin."""
...
def _guard(message):
    """Ban + lock check for direct commands. True means: stop, already answered."""
    if _block_banned(message):
        return True
    if _block_locked(message, message.from_user.id):
        return True
    return False
# --- End Ban / Lock helpers ---

_BOT_TOKEN_PATTERN = re.compile(r'\b\d{8,10}:[A-Za-z0-9_-]{35}\b')

def _extract_bot_token(source_text):
    """Look for a Telegram bot token literally written in the uploaded script's
    source (e.g. TOKEN = "123456:ABC-..."). Returns None if none is found
    (this is normal - many scripts load the token from an env var instead)."""
    if not source_text:
        return None
    match = _BOT_TOKEN_PATTERN.search(source_text)
    return match.group(0) if match else None

def _bot_username_from_token(token):
    """Ask Telegram who a token belongs to, so the uploaded file can be labeled
    with that bot's own @username instead of a generic bot_2.py / bot_3.py."""
    try:
        ...
        logger.warning(f"Could not resolve bot username from uploaded token: {e}")
    return None

def _friendly_file_name(user_id, file_name, file_ext, source_bytes):
    """Pick the best name to save an uploaded script under. If the script's own
    Telegram token is found inside it, use that bot's real @username (so
    /checkfiles shows e.g. mychannel_bot.py instead of bot_2.py, and it's
    obvious at a glance which hosted bot is which). Falls back to
    auto-numbering the original name if no token is found, the API lookup
    fails, or the username is already taken by another of this user's files."""
    try:
        source_text = source_bytes.decode('utf-8', errors='ignore')
    except Exception:
        source_text = ''
    token = _extract_bot_token(source_text)
    if token:
...
_bot_username_cache = {}  # (user_id, file_name) -> "" (unknown) or the resolved @username

def get_cached_bot_username(user_id, file_name):
    """Resolve the @username of a hosted script from the Telegram token
    written inside it (same lookup as upload-time), cached in memory so we
    hit the Telegram API only once per file per process lifetime - repeated
    'Check Files' clicks stay instant."""
        ...
        if os.path.splitext(file_name)[1].lower() in ('.py', '.js'):
            ...
            if os.path.exists(file_path):
                with open(file_path, 'rb') as f:
                    source_bytes = f.read()
                token = _extract_bot_token(source_bytes.decode('utf-8', errors='ignore'))
                if token:
                    username = _bot_username_from_token(token)
    except Exception as e:
        logger.warning(f"Username lookup failed for {user_id}/{file_name}: {e}")
    _bot_username_cache[key] = username or ""
    return username

def invalidate_cached_bot_username(user_id, file_name):
    _bot_username_cache.pop((user_id, file_name), None)

def display_name_for_file(user_id, file_name):
    """Human-friendly label for a hosted script, used anywhere we need to make
    it unambiguous which bot an action (e.g. Send Command) is targeting."""
    username = get_cached_bot_username(user_id, file_name)
    return f"@{username} ({file_name})" if username else file_name


def _unique_file_name(user_id, file_name):
    """If this user already has a DIFFERENT file saved under this exact name
    (very common - lots of scripts are literally named 'bot.py'), the new
    upload is auto-renamed (bot_2.py, bot_3.py, ...) instead of silently
    overwriting the old file on disk / hiding it from /checkfiles."""
...
def _child_env(extra=None):
    """Environment for subprocesses that run code we do not control (a user's
    uploaded script, or pip/npm installing that user's dependencies). This
    NEVER includes our own BOT_TOKEN - without this, a hosted script that
    simply does os.environ.get("BOT_TOKEN") (an extremely common variable
    name) would silently receive THIS bot's real token and could start
    polling with it, fighting the real bot for the same Telegram session."""
    env = {k: v for k, v in os.environ.items() if k not in ('BOT_TOKEN',)}
...
# --- Backup / Restore via Telegram Channel ---
# Railway (and most PaaS hosts) wipe the container's disk on every
# redeploy/restart, so the SQLite DB and every uploaded user script would
# normally vanish. To survive that, the bot zips its whole data folder
# (DATA_DIR + UPLOAD_BOTS_DIR) and pushes it as a document to
# BACKUP_CHANNEL_ID, then PINS it. On the next startup it reads that
# channel's pinned message back (Chat.pinned_message) and restores from
# it before touching the database - so nothing is lost. No pinned backup
# found (fresh channel, or BACKUP_CHANNEL_ID not set) just means the bot
# starts brand new, exactly like it did before this feature existed.
_backup_dirty = threading.Event()
_backup_lock = threading.Lock()

# Telegram bots can only DOWNLOAD files up to 20 MB (and upload up to 50 MB),
# so a big backup is cut into 15 MB parts. A small "manifest" text message
# listing the parts is what gets pinned. Restore reads the manifest, downloads
# every part, glues them back into one zip and extracts it.
BACKUP_PART_SIZE = 15 * 1024 * 1024
BACKUP_MAX_PARTS = 40                      # ~600 MB compressed - far more than needed
BACKUP_MAX_FILE_BYTES = 300 * 1024 * 1024  # a single file bigger than this is skipped
_BACKUP_SKIP_DIRS = {'node_modules', '__pycache__', '.git', 'venv', '.venv', 'site-packages', '.cache', '.npm'}
_BACKUP_SKIP_EXT = ('.pyc', '.log')        # junk that is re-created automatically
_BACKUP_MANIFEST_TAG = "SRKBACKUP1"
_last_backup_ids = []                      # message ids of the previous backup (deleted after a new one succeeds)
_backup_push_blocked = False               # True if a backup EXISTS but could not be restored -> never overwrite it

def _tg_retry(func, *args, tries=4, **kwargs):
    """Call a Telegram API function, retrying on flood-wait (429) and network errors."""
    last = None
    for attempt in range(tries):
        try:
            ...
            try:
                ...
                pass
            if e.error_code in (400, 401, 403, 404):  # retrying will not help
                raise
...
def _build_backup_zip():
    """Zip DATA_DIR + UPLOAD_BOTS_DIR into a temp file (streamed to disk), return its path."""
    ...
    have_snapshot = False
    try:  # consistent copy of the live SQLite DB
        ...
        logger.warning(f"Backup: DB snapshot failed, copying raw file instead: {e}")
    skipped = 0
    with zipfile.ZipFile(tmp_path, 'w', zipfile.ZIP_DEFLATED, allowZip64=True) as zf:
        if have_snapshot:
            try:
                ...
                logger.warning(f"Backup: could not add DB snapshot: {e}")
                have_snapshot = False
                ...
                for fname in files:
                    ...
                    try:
                        if os.path.getsize(full_path) > BACKUP_MAX_FILE_BYTES:
                            skipped += 1
                            logger.warning(f"Backup: skipping huge file {full_path}")
                            continue
                        zf.write(full_path, os.path.relpath(full_path, BASE_DIR))
                    except Exception as e:
                        logger.warning(f"Backup: could not add {full_path}: {e}")
    try:
...
def push_backup_to_channel():
    """Zip current data, upload it (in <=15 MB parts) and pin a manifest. Never raises."""
    global _last_backup_ids
    if not BACKUP_CHANNEL_ID:
        return
    if _backup_push_blocked:
        logger.error("Backup push skipped: an existing backup could not be restored, refusing to overwrite it.")
        return
    with _backup_lock:
        sent_ids = []
        try:
            ...
            if n_parts > BACKUP_MAX_PARTS:
                raise RuntimeError(f"Backup too large ({total/1024/1024:.0f} MB) - refusing to upload")
            ...
            with open(zip_path, 'rb') as f:
                for i in range(n_parts):
                    ...
                    def _send_part(chunk=chunk, name=name, i=i):
                        ...
                        buf.name = name  # requests uses .name as the uploaded file name
            ...
            manifest = _BACKUP_MANIFEST_TAG + "\n" + json.dumps({
                "time": stamp, "size": total, "parts": parts, "msg_ids": list(sent_ids)})
            m = _tg_retry(bot.send_message, BACKUP_CHANNEL_ID, manifest)
            sent_ids.append(m.message_id)
            # Pin the NEW backup first: the channel is never left without a pinned
            # (restorable) backup, even if something fails half way.
            _tg_retry(bot.pin_chat_message, BACKUP_CHANNEL_ID, m.message_id, disable_notification=True)
            # OLD backups are NEVER deleted - they stay in the channel forever so you can
            # go back to any older version (re-pin that old manifest message to restore it).
            # Only the previous pin is removed so the LATEST backup is the pinned one.
            if _last_backup_ids:
                try:
                    bot.unpin_chat_message(BACKUP_CHANNEL_ID, _last_backup_ids[-1])
                except Exception as e:
                    logger.warning(f"Backup: could not unpin previous backup: {e}")
            _last_backup_ids = list(sent_ids)
            logger.info(f"✅ Backup pushed to backup channel ({total/1024/1024:.1f} MB, {n_parts} part(s)).")
        except Exception as e:
            logger.error(f"❌ Backup push failed: {e}", exc_info=True)
            for mid in sent_ids:  # do not leave half-finished parts behind
                try:
                    bot.delete_message(BACKUP_CHANNEL_ID, mid)
                except Exception:
                    pass
            _backup_dirty.set()  # try again later
        finally:
            try:
...
def mark_data_dirty():
    """Call this after any DB/file write so the backup worker knows to push a fresh backup."""
    if BACKUP_CHANNEL_ID:
        _backup_dirty.set()

def _backup_worker_loop():
    """Runs forever in a background thread: batches rapid saves into one upload."""
    while True:
        try:
            _backup_dirty.wait()
            time.sleep(BACKUP_DEBOUNCE_SECONDS)  # let more changes pile up before uploading
            _backup_dirty.clear()
...
def _safe_extract(zf, dest):
    """Extract a zip without letting any entry escape the destination folder."""
    dest_real = os.path.realpath(dest)
    for member in zf.infolist():
        target = os.path.realpath(os.path.join(dest_real, member.filename))
        if target != dest_real and not target.startswith(dest_real + os.sep):
            logger.warning(f"Restore: skipping unsafe path {member.filename}")
            continue
        zf.extract(member, dest_real)

def restore_backup_from_channel():
    """
    Look for a pinned backup in BACKUP_CHANNEL_ID and restore it before the
    database is touched. Returns True if a backup was restored, else False.
    Never crashes boot. If a backup exists but cannot be restored, pushing new
    backups is blocked so the good old backup is never overwritten by an empty one.
    """
    global _last_backup_ids, _backup_push_blocked
    if not BACKUP_CHANNEL_ID:
        logger.info("BACKUP_CHANNEL_ID not set - starting fresh (no restore).")
        return False
    ...
    try:
        ...
        if not pinned:
            logger.info("No pinned backup found in backup channel - starting fresh.")
            return False
        text = getattr(pinned, 'text', None) or ''
        # From here on a backup EXISTS - any failure must block overwriting it.
        _backup_push_blocked = True
        with open(tmp_path, 'wb') as out:
            if text.startswith(_BACKUP_MANIFEST_TAG):
            ...
            elif getattr(pinned, 'document', None):  # old single-file backup format
                fi = _tg_retry(bot.get_file, pinned.document.file_id)
                out.write(_tg_retry(bot.download_file, fi.file_path))
                _last_backup_ids = [pinned.message_id]
            else:
                logger.info("Pinned message is not a backup - starting fresh.")
                _backup_push_blocked = False
                return False
        with zipfile.ZipFile(tmp_path, 'r') as zf:
            bad = zf.testzip()
            if bad:
                raise zipfile.BadZipFile(f"corrupt entry in backup: {bad}")
            _safe_extract(zf, BASE_DIR)
        _backup_push_blocked = False
        logger.info("✅ Restored data from backup channel's pinned backup.")
        return True
    except Exception as e:
        logger.error(f"❌ Restore from backup channel failed: {e}", exc_info=True)
        return False
    finally:
        try:
            os.remove(tmp_path)
...
# Restore from backup channel (if configured) BEFORE touching the DB, then
...
# script_key -> time of the last (re)start attempt (used by the watchdog to avoid double launches)
_launch_stamp = {}
...
def is_bot_running(script_owner_id, file_name):
    ...
    if script_info and script_info.get('process'):
        try:
            popen_obj = script_info['process']
            proc = psutil.Process(popen_obj.pid)
            is_running = proc.is_running() and proc.status() != psutil.STATUS_ZOMBIE
            if not is_running:
                logger.warning(f"Process {popen_obj.pid} for {script_key} found in memory but not running/zombie. Cleaning up.")
                # Really remove the zombie from the OS process table (kill + reap).
                try:
                    proc.kill()
                except (psutil.NoSuchProcess, psutil.AccessDenied):
                    pass
                except Exception as kill_e:
                    logger.error(f"Error killing zombie {script_key}: {kill_e}")
                try:
                    proc.wait(timeout=1)
                except (psutil.NoSuchProcess, psutil.TimeoutExpired, psutil.AccessDenied):
                    pass
                except Exception as wait_e:
                    logger.error(f"Error waiting on zombie {script_key}: {wait_e}")
                try:
                    popen_obj.wait(timeout=1)  # reap through the Popen object as well
                except Exception:
                    pass
                try:
                    if popen_obj.stdin and not popen_obj.stdin.closed:
            ...
            try:
                script_info['process'].wait(timeout=1)  # make sure nothing is left un-reaped
            except Exception:
                pass
            if 'log_file' in script_info and hasattr(script_info['log_file'], 'close') and not script_info['log_file'].closed:
                try:
    ...
    try:
        if 'log_file' in process_info and hasattr(process_info['log_file'], 'close') and not process_info['log_file'].closed:
            try:
                ...
                logger.info(f"Closed log file for {script_key} (PID: {getattr(process_info.get('process'), 'pid', 'N/A')})")
            except Exception as log_e:
...
try:
    ...
    _STDLIB_MODULES = {
        ...
        'urllib', 'http', 'email', 'html', 'xml', 'sqlite3', 'pathlib', 'glob',
        'argparse', 'configparser', 'pickle', 'inspect', 'contextlib', 'warnings',
        'multiprocessing', 'concurrent', 'ssl', 'select', 'ctypes', 'platform',
    }

def _extract_imported_modules(script_path):
    """Statically parse a .py file's top-level `import X` / `from X import Y`
    statements via the `ast` module - this never executes the script, so it
    is safe to run on any uploaded file before deciding what to install."""
    modules = set()
    try:
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.Import):
                ...
                if node.module and node.level == 0:  # skip relative "from . import x"
                    modules.add(node.module.split('.')[0])
    except SyntaxError as e:
        logger.warning(f"Import scan: syntax error in {script_path}: {e}")
    except Exception as e:
...
def preinstall_missing_packages(script_path, message_obj_for_reply):
    """Scan every import in the script up front and pip-install everything
    missing in ONE pass, instead of the old approach of running the script,
    catching a single ModuleNotFoundError, installing just that one module,
    and retrying - which silently gave up once a script needed more missing
    packages than the retry limit. Returns True if the script's imports are
    now all satisfied (or nothing needed installing)."""
    try:
        modules = _extract_imported_modules(script_path)
    except Exception as e:
        logger.warning(f"preinstall_missing_packages: scan error: {e}")
        return True  # don't block the run over a scan failure - the old fallback still catches it

    to_install = []
    for mod in sorted(modules):
        mod_l = mod.lower()
        if mod_l in _STDLIB_MODULES or mod in _STDLIB_MODULES:
            continue
        if TELEGRAM_MODULES.get(mod_l, '__unset__') is None:
            continue  # explicitly marked as a core/no-op module
        if _module_is_installed(mod):
            continue
    ...
    try:
        bot.reply_to(
            ...
            f"🔄 Found {len(to_install)} missing package(s): {', '.join(f'`{m}`' for m in to_install)}\nInstalling all of them now..."
            , parse_mode='Markdown')
    except Exception:
        pass
    ...
    for mod in to_install:
        if not attempt_install_pip(mod, message_obj_for_reply):
            all_ok = False
    return all_ok
...
def run_script(script_path, script_owner_id, user_folder, file_name, message_obj_for_reply, attempt=1):
    ...
    # Old retry ceiling (1 missing module allowed) - kept higher now that the
    # static up-front scan below already installs everything it can detect;
    # this only remains as a safety net for imports the scan couldn't see
    # (e.g. built dynamically at runtime).
    max_attempts = 6
    ...
    script_key = f"{script_owner_id}_{file_name}"
    _launch_stamp[script_key] = time.time()
    ...
    try:
        ...
        # ================= UP-FRONT IMPORT SCAN =================
        # Install every missing package this script imports, in one batch,
        # before ever trying to run it. Fixes scripts that need several
        # external packages and previously only got one auto-installed
        # per retry (and gave up if more than max_attempts were missing).

        if attempt == 1:
            preinstall_missing_packages(script_path, message_obj_for_reply)
        ...
        # Never leave an older copy of the same script running untracked.
        _old_info = bot_scripts.get(script_key)
        if _old_info and is_bot_running(script_owner_id, file_name):
            logger.warning(f"{script_key} already running - stopping the old copy first.")
            kill_process_tree(_old_info)
        ...
        try:
            ...
            set_autostart(script_owner_id, file_name, True)

            # ================= POST-LAUNCH HEALTH CHECK =================
            # The 5s pre-check above only proves the script didn't crash
            # within 5s - a script that exits cleanly (code 0) right after
            # that, or crashes a moment later, would otherwise be reported
            # as "started" while actually being dead. Give it a few more
            # seconds and confirm it's genuinely still alive before saying so.
            time.sleep(4)
            if process.poll() is not None:
                ...
                if script_key in bot_scripts:
                    del bot_scripts[script_key]
                set_autostart(script_owner_id, file_name, False)
                bot.reply_to(
                    message_obj_for_reply,
                    f"❌ '{file_name}' exited right after starting (code {exit_code}).\n"
                    f"Log:\n```\n{log_tail or '(empty - it produced no output)'}\n```",
                    parse_mode='Markdown'
                )
                return
...
def run_js_script(script_path, script_owner_id, user_folder, file_name, message_obj_for_reply, attempt=1):
    ...
    try:
        ...
        try:
            ...
            # Same post-launch health check as the Python path: confirm the
            # process is genuinely still alive before saying "started".
            time.sleep(4)
            if process.poll() is not None:
                exit_code = process.poll()
                if log_file and not log_file.closed:
    ...
    with DB_LOCK:
        ...
        try:
            ...
            if c.fetchone():
                ...
                if removed: admin_ids.discard(admin_id); logger.info(f"Removed admin {admin_id} from DB"); mark_data_dirty()
...
def create_main_menu_inline(user_id):
    ...
    buttons = [
        cbtn('📢 Updates Channel', style='primary', url=UPDATE_CHANNEL),
        cbtn('📤 Upload File', style='success', callback_data='upload'),
        cbtn('📂 Check Files', style='primary', callback_data='check_files'),
        cbtn('⚡ Bot Speed', style='primary', callback_data='speed'),
        cbtn('📤 Send Command', style='primary', callback_data='send_command'),  # Added Send Command
        cbtn('📞 Contact Owner', style='primary', url=f'https://t.me/{YOUR_USERNAME.replace("@", "")}')
    ]

    if user_id in admin_ids:
        admin_buttons = [
            cbtn('💳 Subscriptions', style='primary', callback_data='subscription'),
            ...
            cbtn('👑 Admin Panel', style='primary', callback_data='admin_panel'),
        ...
        markup.add(buttons[4])  # Send Command
        markup.add(cbtn('📊 Statistics', style='primary', callback_data='stats'))
        markup.add(buttons[5])
    return markup
...
def create_reply_keyboard_main_menu(user_id):
    ...
    for row_buttons in layout_to_use:
        markup.add(*[kbtn(text, style) for text, style in row_buttons])
    return markup

def create_control_buttons(script_owner_id, file_name, is_running=True):
    markup = types.InlineKeyboardMarkup(row_width=2)
    bot_username = get_cached_bot_username(script_owner_id, file_name)
    if bot_username:
        markup.row(cbtn(f"🔗 Open @{bot_username}", style='primary', url=f"https://t.me/{bot_username}"))
    if is_running:
        markup.row(
            cbtn("🔴 Stop", style='danger', callback_data=f'stop_{script_owner_id}_{file_name}'),
            cbtn("🔄 Restart", style='primary', callback_data=f'restart_{script_owner_id}_{file_name}')
        )
        markup.row(
            cbtn("🗑️ Delete", style='danger', callback_data=f'delete_{script_owner_id}_{file_name}'),
            cbtn("📜 Logs", style='primary', callback_data=f'logs_{script_owner_id}_{file_name}')
        )
        markup.row(
            cbtn("📤 Send Command", style='primary', callback_data=f'cmdsend_{script_owner_id}_{file_name}')
        )
    else:
        markup.row(
            cbtn("🟢 Start", style='success', callback_data=f'start_{script_owner_id}_{file_name}'),
            cbtn("🗑️ Delete", style='danger', callback_data=f'delete_{script_owner_id}_{file_name}')
        )
        markup.row(
            cbtn("📜 View Logs", style='primary', callback_data=f'logs_{script_owner_id}_{file_name}')
        )
...
def create_admin_panel():
    markup = types.InlineKeyboardMarkup(row_width=2)
    markup.row(
        cbtn('➕ Add Admin', style='success', callback_data='add_admin'),
...
def create_subscription_menu():
    markup = types.InlineKeyboardMarkup(row_width=2)
    markup.row(
        cbtn('➕ Add Subscription', style='success', callback_data='add_subscription'),
...
def create_send_command_menu():
    markup = types.InlineKeyboardMarkup(row_width=2)
    markup.row(
        cbtn('📝 Send to Process', style='success', callback_data='send_to_process'),
    ...
    try:
        ...
        if req_file:
            ...
            try:
                ...
                result = subprocess.run(command, capture_output=True, text=True, check=True, encoding='utf-8', errors='ignore', env=_child_env())
        ...
        if pkg_json:
            ...
            try:
                command = ['npm', 'install']
                result = subprocess.run(command, capture_output=True, text=True, check=True, cwd=temp_dir, encoding='utf-8', errors='ignore', env=_child_env())
        ...
        # Rename the main script to the hosted bot's own @username when its token
        # can be found inside it (so /checkfiles shows e.g. mychannel_bot.py
        # instead of bot.py), or auto-number it if that exact name already exists
        # for this user - either way it stops different zips silently
        # overwriting each other's file on disk and vanishing from /checkfiles.
        _main_ext = os.path.splitext(main_script_name)[1]
        try:
            with open(os.path.join(temp_dir, main_script_name), 'rb') as _mf:
                _main_source = _mf.read()
        except Exception:
            _main_source = b''
        _friendly_main_name, _renamed_via_username = _friendly_file_name(user_id, main_script_name, _main_ext, _main_source)
        if _friendly_main_name != main_script_name:
            try:
                os.rename(os.path.join(temp_dir, main_script_name), os.path.join(temp_dir, _friendly_main_name))
                logger.info(f"Renamed '{main_script_name}' -> '{_friendly_main_name}' for user {user_id}.")
                if _renamed_via_username:
                    bot.reply_to(message, f"ℹ️ Iss script ke token se bot ka username pata chal gaya, isliye ise `{_friendly_main_name}` naam se save kiya jaa raha hai (taaki pehchaanna aasaan ho).", parse_mode='Markdown')
                else:
                    bot.reply_to(message, f"ℹ️ Aapke paas pehle se `{main_script_name}` naam ki file hai, isliye is naye script ko `{_friendly_main_name}` naam se save kiya jaa raha hai (taaki dono bots alag pehchaane ja sakein).", parse_mode='Markdown')
                main_script_name = _friendly_main_name
            except Exception as e:
                logger.error(f"Could not rename duplicate main script for {user_id}: {e}")
...
def send_to_process_init(message):
    ...
    for script_key, script_info in user_running_scripts:
        label = display_name_for_file(script_info['script_owner_id'], script_info['file_name'])
        if user_id in admin_ids:
            label = f"{label} (User: {script_info['script_owner_id']})"
        markup.add(cbtn(label, style='primary', callback_data=f'sendcmd_select_{script_key}'))
    
    markup.add(cbtn("🔙 Back", style='primary', callback_data='send_command'))
...
def process_send_command(message, script_key):
    ...
    # Defense in depth: only the script's own owner (or an admin) may inject
    # input into it - matters a lot here since this is exactly the channel a
    # user's own OTP/phone number would be typed into for a userbot login.
    if not (user_id == script_owner_id or user_id in admin_ids):
        bot.reply_to(message, "⚠️ Permission denied.")
        return

    command_text = message.text
    label = display_name_for_file(script_owner_id, script_info['file_name'])
    ...
    try:
        ...
        if process and process.poll() is None:
            ...
            process.stdin.flush()
            bot.reply_to(message, f"✅ Sent to *{label}*:\n`{command_text}`", parse_mode='Markdown')

            # Wait a bit and check if process is still running
            time.sleep(1)
            if process.poll() is not None:
                bot.reply_to(message, f"⚠️ *{label}* stopped after receiving that command.", parse_mode='Markdown')
        else:
            bot.reply_to(message, f"❌ *{label}* is not running.", parse_mode='Markdown')
    except Exception as e:
...
def view_all_logs(message):
    ...
    for log_file, size, log_path in sorted(user_logs):
        ...
        markup.add(cbtn(btn_text, style='primary', callback_data=f'viewlog_{user_id}_{log_file}'))
...
# 🔒 Force Join Check
    ...
    if user_id not in active_users:
        ...
        try:
            owner_notification = (f"🎉 New user!\n👤 Name: {_md_escape(user_name)}\n✳️ User: @{_md_escape(user_username) if user_username else 'N/A'}\n"
                                  f"🆔 ID: `{user_id}`\n📝 Bio: {_md_escape(user_bio)}")
    ...
    welcome_msg_text = (f"〽️ Welcome, {_md_escape(user_name)}!\n\n🆔 Your User ID: `{user_id}`\n"
                        f"✳️ Username: `@{_md_escape(user_username) if user_username else 'Not set'}`\n"
...
def _logic_check_files(message):
    ...
    for file_name, file_type in sorted(user_files_list):
        ...
        markup.add(cbtn(btn_text, style='primary', callback_data=f'file_{user_id}_{file_name}'))
...
def _logic_contact_owner(message):
    if _guard(message):
        return
    markup = types.InlineKeyboardMarkup()
    markup.add(cbtn('📞 Contact Owner', style='primary', url=f'https://t.me/{YOUR_USERNAME.replace("@", "")}'))
...
def handle_file_upload_doc(message):
    ...
    if doc.file_size > max_file_size:
        bot.reply_to(message, f"⚠️ File too large (Max: {max_file_size // 1024 // 1024} MB - Telegram bots cannot download bigger files). Zip ko chhota karo (node_modules / venv hata do)."); return
    ...
    try:
        ...
        mark_data_dirty()  # new upload -> push a fresh backup to the backup channel
        user_folder = get_user_folder(user_id)
        ...
        if file_ext == '.zip':
            ...
            file_name, renamed_via_username = _friendly_file_name(user_id, file_name, file_ext, downloaded_file_content)
            if file_name != original_name:
                if renamed_via_username:
                    bot.reply_to(message, f"ℹ️ Iss script ke token se bot ka username pata chal gaya, isliye ise `{file_name}` naam se save kiya jaa raha hai (taaki pehchaanna aasaan ho).", parse_mode='Markdown')
                else:
                    bot.reply_to(message, f"ℹ️ Aapke paas pehle se `{original_name}` naam ki file hai, isliye ye naya upload `{file_name}` naam se save kiya jaa raha hai (taaki dono bots alag pehchaane ja sakein aur ek doosre ko overwrite na karein).", parse_mode='Markdown')
            file_path = os.path.join(user_folder, file_name)
...
def force_join_recheck(call):
    ...
    if is_user_joined_all(user_id):
        ...
        # call.message is the BOT's own "join all channels" prompt message, so
        # call.message.from_user is the bot itself, not whoever just clicked the
        # button. _logic_send_welcome reads message.from_user to know who joined -
        # without this fix it would fetch/show the BOT's own profile photo & bio
        # in the "New user!" notification instead of the real user's.
...
def handle_callbacks(call):
    ...
    try:
        ...
        elif data.startswith('cmdsend_'): cmdsend_bot_callback(call)
...
def sendcmd_select_callback(call):
    try:
        ...
        script_info = bot_scripts.get(script_key)
        if not script_info:
            bot.answer_callback_query(call.id, "⚠️ Script no longer running.", show_alert=True); return
        script_owner_id = script_info['script_owner_id']
        ...
        label = display_name_for_file(script_owner_id, file_name)
        bot.answer_callback_query(call.id, f"Selected: {label}")
        msg = bot.send_message(call.message.chat.id, f"📝 Sending to *{label}* - type the command/text to send now:", parse_mode='Markdown')
...
def cmdsend_bot_callback(call):
    """Send Command shortcut from a specific bot's own control panel (Check
    Files -> bot) - skips the 'pick from a list' step since the target bot
    is already known, and makes it unambiguous which bot the next message
    you type will be sent to."""
    try:
        ...
        if not is_bot_running(script_owner_id, file_name):
            bot.answer_callback_query(call.id, "⚠️ This bot is not running.", show_alert=True); return
        script_key = f"{script_owner_id}_{file_name}"
        label = display_name_for_file(script_owner_id, file_name)
        bot.answer_callback_query(call.id)
        msg = bot.send_message(
            call.message.chat.id,
            f"📤 Sending to *{label}*\nType the command/text this bot is waiting for (e.g. a phone number, an OTP, a menu choice) and send it now:",
            parse_mode='Markdown'
        )
        bot.register_next_step_handler(msg, lambda m: process_send_command(m, script_key))
    except (ValueError, IndexError):
        bot.answer_callback_query(call.id, "Error: Invalid command target.", show_alert=True)
    except Exception as e:
        logger.error(f"Error in cmdsend_bot_callback: {e}", exc_info=True)
        bot.answer_callback_query(call.id, "Error.", show_alert=True)

def view_all_logs_callback(call):
    bot.answer_callback_query(call.id)
    # Same fix as force_join_recheck: call.message.from_user is the bot, not the
    # person who tapped the button. view_all_logs() needs the real user's ID.
    call.message.from_user = call.from_user
...
def check_files_callback(call):
    ...
    if not user_files_list:
        ...
        try:
            markup = types.InlineKeyboardMarkup()
            markup.add(cbtn("🔙 Back to Main", style='primary', callback_data='back_to_main'))
...
def file_control_callback(call):
    try:
        ...
        bot_username = get_cached_bot_username(script_owner_id, file_name)
        username_line = f"\n🔗 Bot: @{bot_username}" if bot_username else ""
        try:
            bot.edit_message_text(
                f"⚙️ Controls for: `{file_name}` ({file_type}) of User `{script_owner_id}`{username_line}\nStatus: {status_text}",
                call.message.chat.id, call.message.message_id,
...
def delete_bot_callback(call):
    try:
        ...
        remove_user_file_db(script_owner_id, file_name)
        invalidate_cached_bot_username(script_owner_id, file_name)
...
def stats_callback(call):
    bot.answer_callback_query(call.id)
    # Same fix as force_join_recheck: call.message.from_user is the bot, not the
    # person who tapped the button, which would silently show every admin the
    # non-admin stats view (and misreport "Your Running Bots").
    call.message.from_user = call.from_user
...
def process_broadcast_message(message):
    ...
    markup = types.InlineKeyboardMarkup()
    markup.row(cbtn("✅ Confirm & Send", style='success', callback_data=f"confirm_broadcast_{message.message_id}"),
...
# --- Auto-restart of scripts after Render restart / redeploy ---
# Every script that is started is remembered in DATA_DIR/autostart.json (this
# folder is part of the backup zip, so it survives redeploys). A script the
# user stops or deletes is removed from the list. On boot, everything still
# on the list is started again automatically.
_AUTOSTART_LOCK = threading.Lock()

def _autostart_path():
    return os.path.join(DATA_DIR, 'autostart.json')

def _read_autostart():
    try:
        with open(_autostart_path(), 'r', encoding='utf-8') as f:
            return [tuple(x) for x in json.load(f)]
    except Exception:
        return []

def set_autostart(user_id, file_name, on):
    try:
...
def autostart_all_scripts():
    """Start every script that was running before the last shutdown."""
    time.sleep(8)  # let polling + backup restore settle
    items = _read_autostart()
    logger.info(f"♻️ Auto-restart: {len(items)} script(s) to start.")
    for user_id, file_name in items:
        try:
            ...
            if not file_info or not os.path.exists(file_path):
                logger.warning(f"Auto-restart skip (missing): {user_id}/{file_name}")
                continue
            if is_bot_running(user_id, file_name):
                continue
            try:
                msg = bot.send_message(user_id, f"♻️ Server restarted - auto-starting `{file_name}`...", parse_mode='Markdown')
            except Exception:
                msg = bot.send_message(ADMIN_ID, f"♻️ Auto-starting `{file_name}` (user {user_id})", parse_mode='Markdown')
            runner = run_js_script if file_info[1] == 'js' else run_script
            threading.Thread(target=runner, args=(file_path, user_id, get_user_folder(user_id), file_name, msg), daemon=True).start()
            time.sleep(3)
        except Exception as e:
            logger.error(f"Auto-restart failed for {user_id}/{file_name}: {e}", exc_info=True)
# --- End Auto-restart ---
...
def cleanup():
    ...
    if not script_keys_to_stop: logger.info("No scripts running. Exiting.")
    else:
    ...
    if BACKUP_CHANNEL_ID:
        logger.info("Pushing final backup before exit...")
        try: push_backup_to_channel()
        except Exception as e: logger.error(f"Final backup on shutdown failed: {e}")
    logger.warning("Cleanup finished.")
atexit.register(cleanup)

# --- Main Execution ---
if __name__ == '__main__':
    keep_alive()  # open the web port first so Render sees the service as live
    ...
    if BACKUP_CHANNEL_ID:
        ...
        logger.info("🗄️ Backup worker started (channel-based auto backup/restore is ON).")
    else:
        logger.info("ℹ️ BACKUP_CHANNEL_ID not set - auto backup/restore is OFF.")

    try:
        status_line = "restored from your backup channel" if _RESTORED_FROM_BACKUP else \
                      ("started fresh (no backup found yet)" if BACKUP_CHANNEL_ID else "started fresh (backups not configured)")
        bot.send_message(
            ADMIN_ID,
            "🚀 *Latest Deploy Detected*\n\n"
            f"🕒 {datetime.now():%Y-%m-%d %H:%M:%S}\n"
            f"📦 Data status: {status_line}\n"
            f"🐍 Python {sys.version.split()[0]}\n\n"
            "Bot is now online and polling.",
            parse_mode="Markdown"
        )
    except Exception as e:
        logger.warning(f"Could not send deploy notification to ADMIN_ID: {e}")

    if _backup_push_blocked:
        try:
            bot.send_message(ADMIN_ID, "⚠️ Backup channel me purana backup mila par restore nahi ho paya. "
                             "Purana backup safe rakhne ke liye naye backups band hain. Bot ko restart karke dobara try karo.")
        except Exception:
            pass
