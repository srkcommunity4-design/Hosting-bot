...
# --- Configuration ---
# Bot token now comes from an environment variable (Railway: Project ->
# Variables -> BOT_TOKEN=<your token>). Nothing else about how TOKEN is
# used below changes.
TOKEN = os.environ.get("BOT_TOKEN")
if not TOKEN:
    raise RuntimeError(
        "BOT_TOKEN environment variable is not set. "
        "On Railway: open your project -> Variables -> add BOT_TOKEN."
    )
...
BACKUP_DEBOUNCE_SECONDS = 15  # batch rapid-fire saves into one upload
...
def _build_backup_zip():
    """Zip DATA_DIR + UPLOAD_BOTS_DIR into a temp file, return its path."""
...
def push_backup_to_channel():
    """Zip current data and push/pin it in the backup channel. Never raises."""
        ...
        try:
            ...
            with open(zip_path, 'rb') as f:
                sent = bot.send_document(
                    BACKUP_CHANNEL_ID, f,
                    caption=f"🗄️ SRK bot backup - {datetime.now():%Y-%m-%d %H:%M:%S}"
                )
            try:
                bot.unpin_all_chat_messages(BACKUP_CHANNEL_ID)
            except Exception as e:
                logger.warning(f"Backup: could not unpin old backups: {e}")
            bot.pin_chat_message(BACKUP_CHANNEL_ID, sent.message_id, disable_notification=True)
            logger.info("✅ Backup pushed to backup channel.")
        except Exception as e:
...
def restore_backup_from_channel():
    ...
    Any failure here just means "start fresh" - it must never crash boot.
    """
    if not BACKUP_CHANNEL_ID:
    ...
    try:
        ...
        if not pinned or not getattr(pinned, 'document', None):
        ...
        logger.error(f"❌ Restore from backup channel failed, starting fresh: {e}", exc_info=True)
        return False
# --- End Backup / Restore ---
