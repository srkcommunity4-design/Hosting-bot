...
# Two separate envs:
#   _child_env() → user scripts  (WITH sandbox — restricted network)
#   _pkg_env()   → pip / npm      (NO sandbox — full network for installs)
# ─────────────────────────────────────────────────────────────────
...
def _child_env(extra: Optional[dict] = None) -> dict:
    """Env for user-hosted scripts. Sandbox active → only whitelisted hosts."""
    env: dict[str, str] = {}
...
def _pkg_env(extra: Optional[dict] = None) -> dict:
    """Env for pip / npm installs. NO sandbox — installers must reach
    pypi.org / registry.npmjs.org freely. Same for everyone, always fast."""
    env: dict[str, str] = {}
    for k in _SAFE_ENV_KEYS:
        v = os.environ.get(k)
        if v is not None:
