...
def handle_zip_file(downloaded_file_content, file_name_zip, message):
    ...
    try:
        ...
        # If this user already has a DIFFERENT saved file with this exact name
        # (very common - many zips contain a script literally called 'bot.py'),
        # rename THIS one before it lands in user_folder, instead of silently
        # overwriting the old file on disk and making it vanish from /checkfiles.
        _unique_main_name = _unique_file_name(user_id, main_script_name)
        if _unique_main_name != main_script_name:
            try:
                os.rename(os.path.join(temp_dir, main_script_name), os.path.join(temp_dir, _unique_main_name))
                logger.info(f"Renamed '{main_script_name}' -> '{_unique_main_name}' for user {user_id} to avoid overwriting an existing file.")
                bot.reply_to(message, f"ℹ️ Aapke paas pehle se `{main_script_name}` naam ki file hai, isliye is naye script ko `{_unique_main_name}` naam se save kiya jaa raha hai (taaki dono bots alag pehchaane ja sakein).", parse_mode='Markdown')
                main_script_name = _unique_main_name
            except Exception as e:
...
def handle_file_upload_doc(message):
    ...
    if file_ext in ['.py', '.js']:
        _unique_name = _unique_file_name(user_id, file_name)
        if _unique_name != file_name:
            bot.reply_to(message, f"ℹ️ Aapke paas pehle se `{file_name}` naam ki file hai, isliye ye naya upload `{_unique_name}` naam se save kiya jaa raha hai (taaki dono bots alag pehchaane ja sakein aur ek doosre ko overwrite na karein).", parse_mode='Markdown')
