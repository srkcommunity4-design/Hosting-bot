    ...
    with _backup_lock:
        ...
        try:
            ...
            try:
                ...
                logger.warning(f"Backup: could not unpin old backups: {e}")
            _tg_retry(bot.pin_chat_message, BACKUP_CHANNEL_ID, m.message_id, disable_notification=True)
            # new backup is safely pinned -> remove the previous one to keep the channel tidy
            for old_id in _last_backup_ids:
                try:
                    bot.delete_message(BACKUP_CHANNEL_ID, old_id)
                except Exception:
                    pass
