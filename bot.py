...
Sirf BOT_TOKEN env se — baaki sab config is file me.
"""
...
# ─────────────────────────────────────────────────────────────────
# NETWORK SANDBOX (child processes only)
# ─────────────────────────────────────────────────────────────────
...
allowed_users_ids: set[int] = set()          # [NEW: Private Access Control]
...
# [NEW: Private Access Control]
def has_access(user_id: int) -> bool:
    """True if user is Owner, Admin, or in allowed_users_ids."""
    if user_id == OWNER_ID:
        return True
    with _lock_state:
...
def _guard(message) -> bool:
    if _block_banned(message):
        return True
    if _block_no_access(message):        # [NEW: Private Access Control]
        return True
...
# ─────────────────────────────────────────────────────────────────
# DATABASE (thread-local connection pool)
# ─────────────────────────────────────────────────────────────────
...
# ─────────────────────────────────────────────────────────────────
# RENAME FILE IN DB — UPDATE, not DELETE
# ─────────────────────────────────────────────────────────────────
...
def cmd_ac(message):
    ...
    safe_reply(message,
        ...
        f"They can now use the bot as a <b>Free User</b>.")
    try:
        safe_send(uid,
            "✅ <b>Access granted!</b>\n\n"
...
def cmd_listac(message):
    ...
    if not ids:
        ...
        return
    txt = f"📋 <b>Allowed Users</b> ({len(ids)}):\n" + "\n".join(
...
def _handle_button_text(message):
    if _block_banned(message):
        return
    if not has_access(message.from_user.id):     # [NEW: Private Access Control]
        safe_reply(
            message,
            "🚫 <b>This is a private bot.</b>\n\n"
            "You do not have access to use it.\n"
...
# ─────────────────────────────────────────────────────────────────
# [FIXED: Regression Fix — restored background upload worker]
...
# [FIXED: Blocking I/O on Main Thread — Change File]
def _process_change_file(message, owner: int, old_fn: str, ft: str):
    """Validation phase — fast, main thread. Heavy work goes to worker."""
    user_id = message.from_user.id
...
def _change_file_worker(message, owner: int, old_fn: str, ft: str):
    """Download, scan, stop old, atomic write, restart — all off-main."""
    user_id = message.from_user.id
    chat_id = message.chat.id
    doc = message.document
    submitted_name = doc.file_name or ""
